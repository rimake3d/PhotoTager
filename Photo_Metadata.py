...
with tabPlaygoround:
    if "free_counter" not in st.session_state:
        st.session_state.free_counter = 5
    no_free = 5
    ...
    with st.container(border = True):
        ...
        colAPI, colUpload = st.columns([1, 5])
        with colAPI:
            st.markdown("""#### Try for FREE in Playground""")
            st.markdown("""##### Start By uploading images""")
            
        with colUpload:
            free_uploaded_files = st.file_uploader("Upload Images",  type=["jpeg", "png","jpg"], accept_multiple_files=True, key="free_basic_gen")
    col_show, col_show_len, col_empty_1 = st.columns([3, 3, 7])
    with col_show:
        show_button = st.button("Show Uploaded Images", key="free_basic_gen_01", help="Click to show uploaded images")
    with col_show_len:
        st.write(f"Number of uploaded images: {len(free_uploaded_files)}")

    if free_uploaded_files is not None and show_button:
        for free_uploaded_file in free_uploaded_files:
            st.image(free_uploaded_file, caption=f"Uploaded {free_uploaded_file.name[0:40]}", width=200)

    with st.container(border = True):
        st.markdown("#### Assemble a prompt for GPT-4 Vision")
        st.markdown("This is to help you write your own prompt for GPT-4 Vision")
        ...
        col_free_text, col_free_prompt = st.columns([1, 1])
        with col_free_text:
            st.text_area("Generated Prompt - To update Press CTRL+ENTER !", height=350, value=f"""{prompt_play}""")
        with col_free_prompt:
            st.markdown("#### Check prompt going into GPT-4 Vision")
            st.write(str(prompt_structure))
        
    with st.container(border = True):
        st.markdown(f"""#### Generated Titles, Descriptions and Keywords - You have {st.session_state.free_counter} free credits Left""")
        free_button = ""
        if not free_uploaded_files:
            free_a_button = st.button("If you want to generate Titles,.. --> Upload Images", key="free_a_button")
            if free_a_button:
                st.write("Upload Images ")
        elif st.session_state.free_counter <= 0:
            st.markdown("#### You have used all your free credits")
            
        else:
            free_button = st.button("Generate Photo Titles, Descriptions", key="free_button" )

    with st.container(border = True):
        if free_uploaded_files and free_button  and len(free_uploaded_files) > 0:
            for free_uploaded_file in free_uploaded_files:
                with st.spinner('Wait for it... Generating titles and descriptions...'):
                    start_time = time.time()
                    title_auto_beta, description, keywords, price = generate_image_metadata(free_uploaded_file, prompt_structure, st.session_state.Free_API_Key)
                    st.session_state.free_counter -= 1
                    if st.session_state.free_counter == 0:
                        st.markdown("#### You have used all your free credits")
                        break
                    col1, col2 = st.columns([1, 12])
                    with col1:
                        st.image(free_uploaded_file, caption=f"Uploaded {free_uploaded_file.name[-20:]}", width=150)
                    with col2:
                        st.write(f"""Title: {title_auto_beta}""")
                        st.write(f"""Description: {description}""")
                        st.write(f"""{free_uploaded_file.name}  |  Title Generated in {proc_time:.2f} seconds  |  Price: ${price}""")
                    end_time = time.time()
                    proc_time = end_time - start_time
                    total_time = total_time + proc_time
                    total_price = total_price + price