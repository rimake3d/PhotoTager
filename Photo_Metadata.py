...
if key_choice == "Your Key":
    ...
    st.sidebar.text_input("First, enter your OpenAI API key", type="password", key="API_Key")

elif key_choice == "Free Key (capped)":
    
    if st.session_state.counter >= no_free:
        st.session_state.API_Key = None
        st.sidebar.caption("First, enter your OpenAI API key")
...
tabMetadata, tabPlaygoround = st.tabs(["Generate Photo Metadata", "Playground"])

with tabMetadata:
    uploaded_files = st.file_uploader("Upload Images",  type=["jpeg", "png","jpg"], accept_multiple_files=True, key="basic_gen")
    ...
    if not uploaded_files and not st.session_state.API_Key:
        a_button = st.button("Upload Images and Enter API key", key="basic_gen_02")
        if a_button:
            st.write("Upload Images and Enter API key")
    elif not uploaded_files:
        b_button = st.button("Upload Images", key="basic_gen_03")
        if b_button:
            st.write("Upload Images")
    elif not st.session_state.API_Key:
        c_button = st.button("Enter API key", key="basic_gen_04")
        if c_button:
    ...
    if uploaded_files and button and st.session_state.API_Key:
        for uploaded_file in uploaded_files:
            with st.spinner('Wait for it... Generating titles and descriptions...'):
                ...
                st.session_state.counter += 1
                if st.session_state.counter >= no_free and not st.session_state.get('api_key_prompt_shown', False):
                # Mark that the user has been prompted for the API key
                    st.session_state['api_key_prompt_shown'] = True
                    st.session_state.API_Key = None
                    st.sidebar.caption("You used free keys - Enter your OpenAI API key to continue")
                    image_arrow = st.sidebar.image("Gifs/arrow_small_new.gif")
